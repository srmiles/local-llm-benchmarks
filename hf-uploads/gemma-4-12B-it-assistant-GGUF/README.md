...
# Gemma 4 12B-it Assistant (MTP Drafter) — GGUF (BF16)

Correctly-converted **BF16 GGUF** of Google's official [`google/gemma-4-12B-it-assistant`](https://huggingface.co/google/gemma-4-12B-it-assistant) MTP (Multi-Token Prediction) drafter, for use with [llama.cpp](https://github.com/ggml-org/llama.cpp) speculative decoding.

Pair this drafter with the [Gemma 4 12B](https://huggingface.co/google/gemma-4-12B-it) target model for a substantial decode-throughput uplift with mathematically identical output quality.
...
```bash
llama-server \
  -m gemma-4-12B_q4_0-it.gguf \
  --model-draft gemma-4-12B-it-assistant-official.bf16.gguf \
  --spec-type draft-mtp \
  --spec-draft-n-max 3 \
...
|---|---|
| Decode | **70.4 tok/s** |
| MTP acceptance rate | 69.7% |
| Prefill @ 2K tokens | **1,053 tok/s** |
| VRAM (target + drafter, Q4_0 target + BF16 drafter) | 8.5 GiB |

Compared to 12B without a drafter: **+257% decode, +530% prefill** on b10215.
...
hf download google/gemma-4-12B-it-assistant --local-dir gemma-4-12B-it-assistant-hf
python convert_hf_to_gguf.py gemma-4-12B-it-assistant-hf/ \
  --outfile gemma-4-12B-it-assistant-official.bf16.gguf \
  --outtype bf16
```

## Files

- `gemma-4-12B-it-assistant-official.bf16.gguf` — 862 MB, BF16

## Credits

- **Model weights:** Google — [`google/gemma-4-12B-it-assistant`](https://huggingface.co/google/gemma-4-12B-it-assistant)
