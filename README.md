...
**Bench build (not yet cut over):** `llama.cpp:sycl-f16-b10867-patched-b3ce098f7` — upstream `f3f1a8f27` (**b10867**) + the same 8-commit `ggml-sycl` series, **rebased with zero conflicts**. Built 2026-09-09 because `spark2_5` support landed upstream in [#27868](https://github.com/ggml-org/llama.cpp/pull/27868) on 2026-09-06, two days after the b10809 cutover. Incumbents re-run on it as anchors: **Ornith +1.2% decode / −0.5% prefill, Gemma 4 E2B −2.5% / −0.3%** — flat in opposite directions, so the 58-commit bump moved nothing and production stays on b10809 for now (finding #53).

**Current production llama.cpp build:** `llama.cpp:sycl-f16-b10809-patched-f937ca544` (upstream master `85d5703a3` + the 8-commit local `ggml-sycl` series, #28159 revert dropped) — **cut over 2026-09-04** on Ornith `:8002`, Gemma 4 E2B categorise `:8006` and Nemotron `:8011`. `llamacpp-embed :8004` stays on `llama.cpp:sycl-f16` (b10688-patched `880b848f4`); llm2.local is unchanged. Measured on this bump: **Ornith +25.4% decode** (upstream + the dropped revert, *not* our patches), E2B and Gemma-26B parity, **Nemotron 98.91 tok/s — a new high**. An unpatched control build of the same base (`sycl-f16-b10809-clean-85d5703a3`) is also on disk and puts our patch series at **+17% on E2B, +6% on Nemotron, ~0 on Ornith** (finding #51). [Rebase notes, A/B, control arm and cutover.](models/tested/2026-09-04-b10809-rebase-ab.md) Rollback tags for every previous image are on disk and named in each launcher. [Patch exports.](configs/patches/README.md)
...
- **2026-09-09** — **MiniCPM5-2B + DSpark: 302.65 tok/s, a new repo decode record, and it beats the categorise incumbent on prefill too.** `openbmb/MiniCPM5-2B` Q4_K_M with the official DSpark drafter at `n-max 7 / p-min 0.6` hits **302.65 tok/s at 100.0% acceptance** vs Gemma 4 E2B's 188.06 (**+60.9%**), prefill 3,614 vs 3,328. **Undrafted it prefills at 5,989 @ 2K and 5,029 @ 12K — the highest prefill numbers in this repo**, +51% over E2B, at 5.24 GiB. Plain `llama` arch, so it loads on the current production image with no rebuild. **Q8_0 drafter beats F16** (+6.9% decode at half the size; the 0.2 pp of acceptance F16 buys is worthless) — the head-to-target-ratio rule measured directly for the first time. **n-max ceiling is real and belongs to the drafter**: `accepted_per_draft` saturates at ~6.85 with p-min off as well as on, so 7 is the cap and 9/11 are inert. **`p-min` bought variance, not throughput here** — σ 4.76 → 1.87 at ~0 decode cost, the exact inverse of finding #34's +26% on Nemotron, because that drafter accepted 46% and this one accepts 100%. **Not promoted:** the categorise pair lives on the **12 GiB B580**, where solo at 5.24 GiB is a plausible swap for E2B's 3.96 but drafted at 8.33 GiB likely will not co-reside with embed + rerank — and the solo-vs-drafted choice hinges on a categorise prefill:decode ratio nobody has measured. Functional probe 6/6. [Findings #55–#57.](docs/findings.md) · [Full bench.](models/tested/minicpm5-2b.md) · [Raw results.](configs/benchmarks/20260909-round2/)

- **2026-09-09** — **Spark-X2.5-4B's drafter question closed: it works, at 99.7% acceptance, and is still not worth it.** No MTP head exists and none can be built (290 tensors, layers 0–35, nothing spare), but the vocab-identical `Spark-X2.5-1.7B` sibling drafts via `--spec-type draft-simple` at **98.7 → 99.7% acceptance, returning 4.95 of 5.00 tokens**. **Decode is flat between n-max 3 and 5 (93.67 → 93.79)** while costing **−29% prefill and +3.85 GiB** — so the ceiling is the draft model's own forward cost, not acceptance, and it spends exactly the VRAM advantage Spark had over Ornith. A 2.4× size ratio does not hurt alignment at all; it caps the speedup at ~+7% regardless. Contrast MiniCPM5's 0.35 GB DSpark head scaling 233 → 302 over the same sweep. **Spark's best configuration remains plain undrafted.** [Finding #55.](docs/findings.md) · [Drafter section.](models/tested/spark-x2.5-4b.md)

- **2026-09-09** — **Spark-X2.5-4B: a dense 4B beats the production chat slot on decode, prefill and VRAM — undrafted.** `XHToken/Spark-X2.5-4B` Q4_K_M lands at **87.74 tok/s decode mean vs Ornith 1.5-9B's 77.55 (+13.1%)**, **prefill 3,643 vs 1,892 @ 12K (+92.6%)**, at **6.44 GiB vs 11.33 (−43%)** — with **no drafter at all** against Ornith's Q8_0 MTP head. Decode **σ 0.35** across 20 runs vs Ornith's 13.79: no head means no acceptance bimodality (finding #31), so p99 ≈ median, which matters more for agent latency than a median tok/s. Hybrid 3:1 sliding-window(512)/full attention, 36 layers, 1M native context; **needle retrieval 20/20 across five depths × four lengths out to 106K tokens**, so the nine full-attention layers do carry long-range recall. Functional probe 6/6 on both quants — tool calling, `enable_thinking`, `reasoning_content` separation and structured JSON all work under `--jinja` unchanged. **Q8_0 rejected: −35.9% decode for +1.65 GiB**, a 1.56 throughput ratio against a 1.68 size ratio, so decode is bandwidth-bound and the quant is the dial. **Not promoted** — needs a `docs/track2` bake-off on the pi.dev corpus first; this is a different architecture, tokenizer and publisher from the incumbent, not an Ornith-shaped sibling. Required a new build (below). [Findings #52–#54.](docs/findings.md) · [Full bench.](models/tested/spark-x2.5-4b.md) · [Runner + raw results committed.](configs/benchmarks/20260909-spark-x25/)

- **2026-08-27** — **OpenVINO GenAI: 51 tok/s on B60 for Qwen 3.6-27B-A3B-Coder, replicating marfrit's A770 3× ceiling gap.** Task #148 complete. Ran marfrit's pre-built IR (`marfrit/Qwen3.6-27B-A3B-Coder-int4-awq-se-ov`, 14.8 GB int4 g64) unchanged on our card 2: `openvino 2026.3.1` + `openvino-genai 2026.3.1.0` in a Python 3.14 venv, no conversion, no hacks. **204.5 s cold compile, 51.09 tps decode** (512 tokens greedy, TTFT 1.13 s). Marfrit got 43 tps on A770 → we get **+18.8% at 51 tps on B60** (second-gen Battlemage over Alchemist). Reference — our best llama.cpp SYCL number on the same architecture family is **49 tps for Qwen 3.6-*35B*-A3B-MTP UD-Q4_K_XL**, and marfrit's llama.cpp SYCL on this *exact* 27B was 14.4 tps. So OpenVINO on our B60 for this model is **~3-3.5× llama.cpp SYCL** for the same weights on the same silicon class. Peak VRAM 23 GiB, idle 5 GiB. **Task #142 is now the wrong direction.** Next probe: convert Nemotron 3.5 Lightning to OpenVINO IR ourselves and bench against the current 74 tps on llama.cpp b10567+moereorder; if it clears 100 tps, the reasoning slot migrates permanently. [Finding #47 updated.](docs/findings.md) · [Full bench.](models/tested/2026-08-27-openvino-qwen36-27b-a3b-coder.md)
...
| [**MiniCPM5-2B + DSpark**](models/tested/minicpm5-2b.md) ⭐ (b10867) | Q4_K_M + DSpark Q8_0, n-max 7 / p-min 0.6 | 2.5B dense | **302.65** (σ 1.87) | 3,614 @ 12K | 8.33 GiB | **100.0%** | **benched 2026-09-09** — fastest decode in this repo; VRAM likely blocks it on the 12 GiB B580 |
| [MiniCPM5-2B (solo)](models/tested/minicpm5-2b.md) (b10867) | Q4_K_M, no drafter | 2.5B dense | 137.14 (σ 1.27) | **5,029 @ 12K · 5,989 @ 2K** | 5.24 GiB | n/a | **highest prefill in this repo** — the config to bake off against E2B for categorise |
| [Spark-X2.5-4B + 1.7B draft](models/tested/spark-x2.5-4b.md) (b10867) | Q4_K_M + 1.7B Q4_K_M, `draft-simple` | 4.1B + 1.7B | 93.79 | 2,583 @ 12K | 10.31 GiB | 99.7% | rejected — +6.8% decode for −29% prefill and +60% VRAM (finding #55) |
| [**Spark-X2.5-4B**](models/tested/spark-x2.5-4b.md) ⭐ (b10867) | Q4_K_M, **no drafter** | 4.1B dense | **87.74** (σ 0.35) | **3,643 @ 12K** | **6.44 GiB** | n/a — no MTP exists | **benched 2026-09-09** — beats Ornith 1.5-9B+MTP on decode, prefill and VRAM; needs a quality bake-off before any cutover |
| [Spark-X2.5-4B](models/tested/spark-x2.5-4b.md) (b10867) | Q8_0, no drafter | 4.1B dense | 56.25 | 3,105 @ 12K | 8.09 GiB | n/a | rejected — −35.9% decode vs Q4_K_M for +1.65 GiB |
...
| [Qwen 3.8-27B — unsloth **UD 2.0**](models/tested/2026-08-27-unsloth-dynamic-3-vs-2.md) (`sycl-f16-allfixes`) | UD-Q4_K_XL 17.92 GB + unsloth MTP head | 27B dense hybrid | **18.91** | **711 @ 12K** | 20.36 GiB | 55.2% | fastest of the four arms — **prefer 2.0 over 3.0 when you want tok/s** (finding #44). Pooled over 16 interleaved runs. Not directly comparable to the 23.0 row above: different prompt, sampling and `ignore_eos` |
| [Qwen 3.8-27B — unsloth **UD 3.0**](models/tested/2026-08-27-unsloth-dynamic-3-vs-2.md) (`sycl-f16-allfixes`) | UD-Q4_K_XL 17.56 GB + unsloth MTP head | 27B dense hybrid | 17.11 | 685 @ 12K | 20.02 GiB | 53.5% | **most faithful** to Q8_0 of the four (mean KLD 0.00453) but 9.5% slower to decode — more block types, mechanism unproven |
