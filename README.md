...
All numbers below are measured on the same physical card. The stack has shifted over time — vLLM-XPU → LM Studio Vulkan → llama.cpp Vulkan → llama.cpp SYCL (current) — but the hardware is constant. Unless a row says otherwise, benchmarks were taken on [`llama.cpp:sycl-f16`](configs/images/llama.cpp-sycl-f16/README.md) — **currently on b10215 (commit `eb41d503b`, upgraded 2026-07-31 from b10068)**. Historical bench numbers noted below still reflect the b9948 → b10068 comparison; b10215 delivered no meaningful decode/MTP-acceptance change under real workload (mostly SYCL infrastructure fixes, `gemma4-assistant` arch support for future MTP use, correct `LLAMA_BUILD_NUMBER` reporting, and multi-GPU `dev2dev` memcpy for planned 2nd B60).
